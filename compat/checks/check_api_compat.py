#!/usr/bin/env python3
"""
compat/checks/check_api_compat.py
===================================
CLI tool: check backward-compatibility between two OpenAPI spec files.

Usage
-----
    python check_api_compat.py <old_spec.yaml> <new_spec.yaml>
...
from compat.rules.api_compat_rules import run_all_checks  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) < 2:
        print(
            _colour("Usage: check_api_compat.py <old_spec.yaml> <new_spec.yaml>", "bold"),
            file=sys.stderr,
        )
        return 1
    ...
    try:
        ...
        print(_colour(f"ERROR: Failed to parse specs: {exc}", "red", "bold"), file=sys.stderr)
        return 1
    ...
    if has_failures:
        print(_colour("RESULT: FAILED – API changes are not backward-compatible.", "red", "bold"))
        return 1

    if has_warnings:
        print(_colour(
            "RESULT: PASSED with warnings – review above before merging.",
            "yellow", "bold",
        ))
        return 0

    print(_colour("RESULT: PASSED – API changes are backward-compatible.", "green", "bold"))
    return 0
